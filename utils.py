...
mu_img = torch.tensor(ImageNet_MEAN).view(3, 1, 1)
std_img = torch.tensor(ImageNet_STD).view(3, 1, 1)
...
def load_imagenet_label2folder(path):
    dict_imagenet_folder2name = {}
    with open(path) as f:
        line = f.readline()
        while line:
            ...
            cat_id = split_name[0]
            label = split_name[1]
            dict_imagenet_folder2name[label] = cat_id
            line = f.readline()
    # print(dict_imagenet_folder2name)
    return dict_imagenet_folder2name
